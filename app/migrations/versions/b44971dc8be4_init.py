...
Revision ID: b44971dc8be4
Revises:
Create Date: 2024-10-27 06:50:58.104812
...
from app.extras.models import JSONBPydanticField
from app.models.events import AttendeeQuestion, ContactInformation, EventFee
from app.models.organizations import OrganizationMember

# revision identifiers, used by Alembic.
revision: str = "b44971dc8be4"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "fee",
            JSONBPydanticField(
                pydantic_model_class=EventFee, many=False, astext_type=sa.Text()
            ),
            nullable=True,
        ),
        ...
        sa.Column(
            "attendee_questionnaire",
            JSONBPydanticField(
                pydantic_model_class=AttendeeQuestion, many=True, astext_type=sa.Text()
            ),
            nullable=True,
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        ...
        sa.Column("attended_event", sa.Boolean(), nullable=False),
        sa.Column(
            "email", sqlmodel.sql.sqltypes.AutoString(length=320), nullable=False
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            "contact_information",
            JSONBPydanticField(
                pydantic_model_class=ContactInformation, astext_type=sa.Text()
            ),
            nullable=True,
        ),
        sa.ForeignKeyConstraint(["event_id"], ["events.id"], ondelete="CASCADE"),
